...
const updatePatientResources = async (paciente) => {
  ...
  try {
    ...
    const [actualResources, fields] = await conn.execute(
    ...
    const resourcesToReturn = actualResources.filter(
      (ar) => recursos.filter((r) => r.idRecurso == ar.id).length == 0
    );

    console.log('recursos a devolver: ' + JSON.stringify(resourcesToReturn));

    resourcesToReturn.forEach(async (r) => {
      await conn.execute(
